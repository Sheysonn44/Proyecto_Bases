...
public class InversionDAO {
    ...
    public int insertarYObtenerID(Inversion inv) throws Exception {
        ...
        String sql = "INSERT INTO Inverciones (" +
                "D_Nombre, T_Invercion, M_Invercion, M_Rentabilidad, " +
                "F_Inicio, F_Final, D_Descripcion, " +
                "C_Cuenta_Bancaria, C_Estado, C_TipoInversion, C_TipoMoneda) " +
                "OUTPUT INSERTED.C_Invercion " + // columna PK real
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            stmt.setDate(5, new java.sql.Date(inv.getFechaInicio().getTime())); // F_Inicio
            stmt.setDate(6, new java.sql.Date(inv.getFechaInicio().getTime())); // Para F_Final
            ...
            if (rs.next()) {
                return rs.getInt(1); // C_Invercion generado
            } else {
                throw new Exception("No se generó C_Invercion.");
            }
        }
    }
